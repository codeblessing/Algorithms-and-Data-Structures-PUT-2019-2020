...
pub fn sort(
...
) -> (Vec<i32>, u32, u32, Vec<i32>) {
    let mut sorted = data.clone();

    let mut pivots: Vec<i32> = vec![];
    ...
    unsafe {
        let out = (sorted, COMPARISONS, SWAPS, pivots);
        COMPARISONS = 0;
        SWAPS = 0;
        return out;
    }
...
fn _quicksort_(
    ...
    _quicksort_(array, left, j, compare, pivots);
    _quicksort_(array, i, right, compare, pivots);
}
